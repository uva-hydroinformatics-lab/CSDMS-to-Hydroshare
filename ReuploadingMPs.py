...
import bs4
import requests
# we only want the first 60 resources
numresources = 0
...
auth = HydroShareAuthBasic(username='emorgan117', password="XXXXXXXXXX")
hs = HydroShare(auth=auth)

# getting the current list of resources
listOfResources = hs.getResourceList(owner='emorgan117')
...
for element in names:  # getting the metadata webpage and title for each model
    # only add it to models if it's not already a resource
    numresources += 1
    if element.element.get('title').replace('Model:', '', 1) not in listOfResources and \
            numresources <= 60:
        item = []
...
for model in models:
    ...
    for i in range(0, fields.__len__()):
        ...
        if hydro_fields.__contains__(fields[i].getText().strip('\n').lstrip(' ')):
            ...
            if (fields[i].getText().strip('\n').lstrip(' ') == "Source web address" and "sourceforge" in answers[
                i].getText() or
                            fields[i].getText().strip('\n').lstrip(' ') == "Source web address" and "github" in
                        answers[i].getText()):
            ...
            elif (fields[i].getText().strip('\n').lstrip(' ') == "Source csdms web address" and "sourceforge" in
                answers[i].getText() or
                              fields[i].getText().strip('\n').lstrip(
                                  ' ') == "Source csdms web address" and "github" in answers[i].getText()):
                machinegendata["Software Repository"] = answers[i].getText().strip('\n')
                softwareRepo = True
            else:
